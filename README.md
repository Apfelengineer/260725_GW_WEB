...
PHP APIと内部用SQLiteへデータを保存するため、別端末・別ブラウザ間でも編集内容が共有されます。試験室の空き状況は個人情報を含まない3か月分の公開用JSONへ変換し、署名付きHTTPSで外部公開サーバーへ送信します。スケジューラーはrenkon（社内ポータル）が発行する当日用暗号化トークンを検証して一般モードで開き、パスワード確認後だけ管理者モードへ切り替わります。セッションとCSRF対策はサーバー側で管理します。
...
- ユーザーと予定種別の追加・編集・削除
- renkonの当日用暗号化トークンを確認し、ログイン画面なしで予定表を開く一般モード
...
連携確認用の`renkon`は、入力IDから日本時間の`yyyyMMdd_user_xxx`を作成し、PHPの`openssl_encrypt`で`AES-128-ECB`暗号化して`?token=...`としてoriginへ渡します。originは復号後の日付が当日、IDが半角数字3桁であることを確認し、条件を満たさない画面・API要求には`403 Forbidden`を返します。カレンダーは従来どおり独立した公開リンクです。

指定仕様との互換用既定鍵は`test`です。renkon側の`KPTC_PORTAL_TOKEN_KEY`とorigin側の同名設定を必ず同じ値にし、スケジューラURLはrenkon側の`KPTC_RENKON_SCHEDULER_URL`で変更できます。`config.js`は公開カレンダーURLだけを管理します。本番では既存社内システムにrenkonの`open-scheduler.php`相当処理を組み込み、ネットワーク制限も併用してください。
...
重要: 一般モードでも予定を編集できます。本実装はトークンなしの直接アクセスを403にしますが、指定された`AES-128-ECB`と試験用鍵`test`だけを本番の唯一の防御にしないでください。リバースプロキシ、VPNまたは社内ネットワーク制限も併用してください。
