...
公開画面の試験室一覧は、スケジューラーで所属を「試験室」にしたユーザーから自動生成し、署名付き公開JSONへ名称・画像ファイル名・表示順を含めます。新しい試験室は、管理画面で試験室ユーザーを追加し、外部サーバーの`calendar/`へ`<ユーザーID>.png`を配置すると、次回のJSON送信後に画面へ追加されます。画面のTypeScript修正や再ビルドは不要です。
