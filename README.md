...
PHP APIと内部用SQLiteへデータを保存するため、別端末・別ブラウザ間でも編集内容が共有されます。試験室の空き状況は個人情報を含まない3か月分の公開用JSONへ変換し、署名付きHTTPSで外部公開サーバーへ送信します。スケジューラーはログイン画面を介さず一般モードで開き、パスワード確認後だけ管理者モードへ切り替わります。セッションとCSRF対策はサーバー側で管理します。
...
スケジューラーURLを開くと、ログイン画面を表示せず一般モードの予定表が開きます。一般モードでは予定の閲覧・追加・編集・移動・コピー・削除ができます。画面内の「管理者モード」から管理者パスワードを入力すると、ユーザー・試験室・予定種別の管理機能が追加されます。管理者モードから一般モードへはパスワードなしで戻せます。
...
重要: 一般モードでも予定を編集できます。スケジューラーURLへ社内システムを経由せず直接アクセスできないよう、リバースプロキシ、Webサーバー、VPNまたは社内ネットワーク制限で必ず保護してください。
