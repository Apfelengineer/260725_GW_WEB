...
PHP APIと内部用SQLiteへデータを保存するため、別端末・別ブラウザ間でも編集内容が共有されます。試験室の空き状況は個人情報を含まない3か月分の公開用JSONへ変換し、署名付きHTTPSで外部公開サーバーへ送信します。社内システムでの認証後、カレンダーでは利用するユーザーだけを選択し、権限・セッション・CSRF対策をサーバー側で管理します。
...
- ユーザーと予定種別の追加・編集・削除
- 社内システムでの認証を前提としたユーザー選択ログインと、閲覧専用ゲストログイン
- 管理者・一般・試験室の3権限（管理者は全管理、一般は予定編集、試験室は閲覧のみ）
- ユーザー編集画面に統合したアカウントID・権限管理
...
このカレンダーは、社内システムでユーザー名とパスワードの確認が済んだ後に表示される前提です。カレンダーのログイン画面では利用するユーザーを選ぶだけで、パスワードは入力しません。選択後は従来どおり、管理者・一般・試験室・ゲストの権限とサーバーセッションが適用されます。

初回起動後は、内部サーバー上でスケジューラーのメンバーIDに対応するアカウントを作成します。

```bash
php manage-auth-user-cli.php create admin m1 admin
```

一覧、無効化、再有効化も管理コマンドから行えます。
...
重要: カレンダー単体では、選択した人物が本人かどうかを確認しません。カレンダーのURLへ社内システムを経由せず直接アクセスできないよう、リバースプロキシ、Webサーバー、VPNまたは社内ネットワーク制限で必ず保護してください。管理者を選ぶと管理者権限が付与されるため、このアクセス制限が必須です。

## 共有API

`public/api.php` が内部の共有データ、ユーザー選択ログイン、操作履歴、変更取り消しを提供します。予定を保存・削除・取り消した後、試験室3室の当月を含む3か月分を公開可能な空き状態へ変換し、`public/availability-publisher.php` が外部サーバーへ送ります。連携に失敗しても予定の保存は取り消さず、内部DBへ再送待ち、連続失敗回数、最終試行・成功日時、エラー概要を記録します。
...
公開画面の試験室一覧は、`public/availability-room-config.php`の許可リストに登録した「試験室」ユーザーから生成し、署名付き公開JSONへ名称・画像ファイル名・表示順を含めます。新しい試験室は、管理画面で試験室ユーザーを追加し、同ファイルの`kptc_public_room_ids()`へユーザーIDを追記して、外部サーバーの`calendar/`へ`<ユーザーID>.png`を配置すると、次回のJSON送信後に画面へ追加されます。画面のTypeScript修正や再ビルドは不要です。許可リストにない試験室ユーザーは外部公開されません。

既存3室の公開名称・説明は`public/availability-room-config.php`で上書きしています。スケジューラー上の名称と公開名称を変える場合だけ、この設定を変更してください。画像が未配置の場合は、試験室名の先頭2文字を代替表示します。
